  ...
  return [
    {
      ...
      text: [
        ...
        `  • You CAN show images inline in the conversation — do NOT tell the user "I can only output text" or fall back to opening a browser tab. Emit a markdown image \`![alt](src)\` and it renders inline as a clickable picture (the user can click it to copy to clipboard or save to disk). \`src\` may be an http(s) URL (e.g. a cat photo), a \`file://\` absolute path (a local image file), or a \`data:image/...;base64,...\` URL (an image you have bytes for). So to "show me a picture of X": find/produce an image URL or file and inline it with \`![X](that-url)\`. AppScreenshot results and other image tool outputs the model receives can likewise be re-surfaced to the user by inlining their source.`,
        ``,
